<script lang="ts">
  ...
  import { get_layout } from "../../api/trendmap/get_layout";
...
{#if $addTickerDialog}
  <div
    class="secondary-theme border absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-72 h-16 rounded-sm font-semibold flex items-center justify-between p-3 gap-3"
  >
    <div class="flex items-center w-full justify-around">
      ...
      <div class="relative justify-center flex items-end">
        <input
          on:input={setTicker}
          placeholder="TSLA"
          class="p-1 rounded-md border {$errorMessage !== ''
            ? 'border-red-important'
            : ''} text-sm font-normal transition-all focus:duration-0 w-24"
        />
        {#if $errorMessage !== ""}
          <p class="text-xs absolute -bottom-4 text-red-600">
            {$errorMessage}
          </p>
        {/if}
      </div>
...
{#if $deleteTickerDialog}
  <div
    class="secondary-theme border absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-14 rounded-sm font-semibold flex items-center justify-between p-3 gap-3"
  >
    <div class="flex items-center w-full justify-around">
      <p>
        Delete<span class="text-red-500 pl-1 pr-1">{$ticker}</span>. Are you
        sure?
      </p>
    </div>
    <button
...
<!-- Table of Tickers -->
<div
  class="overflow-y-auto p-9 grid grid-cols-10 max-2xl:grid-cols-8 max-xl:grid-cols-5 max-lg:grid-cols-4 max-md:grid-cols-3 max-sm:grid-cols-2"
>
  {#each $store as { id, name, tickers, hidden }}
    <!-- svelte-ignore a11y-no-static-element-interactions (redudant role) -->
    <section
      class="h-64 w-44 group select-none p-16 relative rounded-lg transition-all duration-200 flex flex-col items-center gap-1 {hidden
        ? ''
        : 'secondary-theme'}"
      ...
      <div class="h-full flex flex-col gap-1.5 p-0.5 overflow-y-auto">
        {#each tickers as { name, mid_term, long_term, short_term }}
          ...
          >
            ...
            ></div>
            <div
              class="secondary-theme w-36 h-8 rounded-md relative border p-1 flex items-center gap-3 border-black"
            >
...
{#if !$addTickerDialog}
  <button
    ...
    class="absolute bottom-0 m-6 w-16 h-8 focus:duration-0 text-sm font-bold rounded-md p-1 active:scale-90 transition-all duration-200"
    >+ New</button
  >
{/if}
